function magicsquareNS_demo()

% This is a guessing-probability optimization rather a feasibility problem, but if the optimal value is 1, this also yields the desired NS hidden-variable model. (Alternatively, one could impose constraints corresponding to the latter and check for feasibility.)
% It uses the NPAHierarchy function from QETLAB (slightly modified to accept subnormalized distributions), which can impose NS or NPA constraints

rng(1); 
tic

% Following NPAHierarchy, npalevel=0 denotes that only the NS constraints should be imposed.
% The resulting LP/SDP is solved in a matter of seconds for npalevel = 0 or 1, but takes about half an hour or longer for npalevel = 2.
npalevel=0

cvx_begin sdp 
variable subnormprobs(4,4,3,3,8) %Using the convention of each party having only 4 possible outputs (i.e. only outputs satisfying the parity constraints are allowed)

numlambda=8; % Number of possible values of lambda. Since only 8 outputs can occur with nonzero probability (for a fixed input pair), it suffices to consider 8 possible values of lambda.

% This code uses the convention of xy for outputs and ab for inputs
akey=1;bkey=1; % Input pair we aim to maximize the guessing probability for
objective=0;
lambda=1;
% For each output pair that can have nonzero probability, we add a term to the objective function, with a corresponding value of lambda.
for x=1:4
for y=1:4
    % Convert x and y to strings, and append the third bit (satisfying parity constraints) 
    xvec=dec2bin(x-1,2)-'0';yvec=dec2bin(y-1,2)-'0'; % str to num witchcraft
    xvec=[xvec mod(sum(xvec),2)];
    yvec=[yvec mod(sum(yvec)+1,2)];
    if xvec(bkey)==yvec(akey) 
        objective = objective + subnormprobs(x,y,akey,bkey,lambda);
        lambda = lambda + 1;
    end 
end
end
...
% Since unnormNPA checks that all distributions are normalized to the same value, it suffices to apply the following constraints to just one of them.
anyinput=1;
% Ensuring normalization, though technically redundant since the full distribution will be imposed as constraints 
sum(vec(subnormprobs(:,:,anyinput,anyinput,:)))==1;
% Ensuring each block is subnormalized and in NPA.
for lambda=1:numlambda 
    sum(vec(subnormprobs(:,:,anyinput,anyinput,lambda)))<=1;
    unnormNPA(subnormprobs(:,:,:,:,lambda),npalevel)==1;
end

% Imposing magic-square distribution as constraints
for a=1:3
for b=1:3
for x=1:4
for y=1:4
    ...
    yvec=[yvec mod(sum(yvec)+1,2)];
    if xvec(b)==yvec(a) 
        sum(vec(subnormprobs(x,y,a,b,:))) == 1/8;
    end 
end
end 
end
...
% View solution directly
format long
subnormprobs
format short

% Remove small terms
cleansol=clean(subnormprobs,10^-5);
cleanedamount=norm(cleansol(:)-subnormprobs(:)) % Checking that not too much was removed

% Distribution of lambda - if the guessing probability is 1, we expect that it must be uniformly distributed (this is the only way for it to satisfy the constraints).
% For NPA levels 1 and 2 (where the guessing probability is < 1), the distribution of lambda seems to be somewhat nonuniform; more so in the latter case.
probslambda=sum(sum(cleansol(:,:,1,1,:),2),1); probslambda=probslambda(:)'; % Flattening the vector

% More convenient display: for each value of lambda, displays a 12x12 matrix showing all the P(xy|ab,lambda) values, with "outer" indexing using the inputs and "inner" indexing using the outputs
% For ease of verification, all entries that do not win the magic square game are replaced with a NaN entry, after confirming that the probability in the original solution was indeed zero.
for lambda=1:8
    soldisplay=zeros(4*3); % This will be the matrix gathering the conditional probabilities P(xy|ab,lambda) for this value of lambda
    for a=1:3
    for b=1:3
    for x=1:4
    for y=1:4
        ...
        xvec=[xvec mod(sum(xvec),2)];
        yvec=[yvec mod(sum(yvec)+1,2)];
        if xvec(b)==yvec(a) % Satisfies win condition
            soldisplay(x+(a-1)*4,y+(b-1)*4)=cleansol(x,y,a,b,lambda)/probslambda(lambda);
        else % Does not satisfy win condition
            if cleansol(x,y,a,b,lambda)>0 % Verify that probability is zero for such terms
                cleansol(x,y,a,b,lambda)
                warning('Win condition not satisfied!')
            else
                soldisplay(x+(a-1)*4,y+(b-1)*4)=NaN;
            end
        end 
    end
    end 
...
%%  unnormNPA    Determines whether or not a set of probabilities satisfy the conditions of the NPA hierarchy
%%  Has been modified from NPAHierarchy() so that it accepts super/subnormalized distributions as well
...
%   IS_NPA = unnormNPA(P) is either 1 or 0, indicating that the
...
%   IS_NPA = unnormNPA(P,K) is as above, but the K-th level of the NPA
...
function is_npa = unnormNPA(p,varargin)


    %%Determining normalization factor based on the x=1,y=1 table
    normfactor = sum(sum(p(:,:,1,1)));
    ...
    elseif(isa(k,'char'))
        ...
        error('unnormNPA:InvalidK','K must be a positive integer or a string.');
    ...
    if(~isa(p,'cvx'))
        ...
        %%Replaced the check for normalization with a check that it is subnormalized and all faces sum to the same value
        for i = 1:ma

            for j = 1:mb

                if(abs(sum(sum(p(:,:,i,j))) - normfactor) > tol^(3/4))
...
%                 stop
                        ...
                        for j = i:real_dim
                            ...
                            elseif(res_type == 1)
                                ...
                                if(isequal(res,[0;-1]))
                                    ...
                                    G(i,j) == normfactor; %%Edited it so the identity measurement takes value normfactor instead of 1
                ...
                if(isa(p,'cvx'))
                    ...
                    for i = 1:ma
                        ...
                        for j = 1:mb
                            ...
                            sum(sum(p(:,:,i,j))) == normfactor; %%Edited to sum to normfactor instead
        ...
        if(~isa(p,'cvx')) % make the output prettier if it's not a CVX input
            ...
            if(strcmpi(cvx_status,'Inaccurate/Solved'))
                ...
                warning('unnormNPA:NumericalProblems','Minor numerical problems encountered by CVX. Consider adjusting the tolerance level TOL and re-running the script.');
            ...
            elseif(strcmpi(cvx_status,'Unbounded') || strcmpi(cvx_status,'Inaccurate/Unbounded') || strcmpi(cvx_status,'Failed'))
                ...
                error('unnormNPA:NumericalProblems',strcat('Numerical problems encountered (CVX status: ',cvx_status,'). Please try adjusting the tolerance level TOL.'));
